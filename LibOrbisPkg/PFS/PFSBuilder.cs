...
namespace LibOrbisPkg.PFS
{
    ...
    public class PfsBuilder
    {
        ...
        void Setup()
        {
            ...
            var temp = FlatPathTable.Create(allNodes);
            fpt = temp.Item1;
            colResolver = temp.Item2;
            ...
            using (var view = file.CreateViewAccessor(offset, CalculatePfsSize(), MemoryMappedFileAccess.ReadWrite))
            {
                if (hdr.Mode.HasFlag(PfsMode.Signed))
                {
                    ...
                    Parallel.ForEach(
                      ...
                      (sig, status, local) =>
                      {
                          var (sig_buffer, hmac) = (local.Item1, local.Item2);
                ...
                if (hdr.Mode.HasFlag(PfsMode.Encrypted))
                {
                    ...
                    var (tweakKey, dataKey) = (temp.Item1, temp.Item2);
                    Parallel.ForEach(
                      ...
                      {
                          var (transformer, sectorBuffer) = (localData.Item1, localData.Item2);
